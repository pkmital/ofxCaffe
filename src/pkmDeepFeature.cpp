...
void pkmDeepFeature::updateFeatures()
{
...
pkm::Mat pkmDeepFeature::getEdgeFeaturesFor(size_t x, size_t y)
{
    // 1st layer is "conv1"
    // 2nd layer is "norm1"
    // 3rd layer is "pool1"
...
void pkmDeepFeature::draw(int w, int h)
{
    caffe->drawGraph(feature_fc5, "conv5", 20, 0, w, h, 255.0f);
    caffe->drawGraph(feature_prob, "prob", 20, 40, w, h, 1.0f);
}

pkm::Mat& pkmDeepFeature::getFeatureFC5()
{
    return feature_fc5;
}

pkm::Mat& pkmDeepFeature::getFeatureProb()
{
    return feature_prob;
}

float pkmDeepFeature::getMaxFeatureProb()
{
    return pkm::Mat::max(feature_prob);
}
